...
public class Start implements Runnable{
    ...
    public Start(MachineRepository machineRepository, Long machineId, Long userId, AsinhronComponent ac, ErrorMsgRepository errorMsgRepository) {
    ...
    public void run() {
        ...
        if (machine.isPresent())
        {
            ...
            if(machine.get().getStatus().equals(Status.RUNNING)){
                ErrorMsg errorMsg = new ErrorMsg(this.machineId, "Machine is already running","Start",new Date(),this.userId);
                errorMsgRepository.save(errorMsg);
                return;
            }
