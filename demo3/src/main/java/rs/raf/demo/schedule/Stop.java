...
public class Stop implements Runnable{
    ...
    public Stop(MachineRepository machineRepository, Long machineId, Long userId, AsinhronComponent ac, ErrorMsgRepository errorMsgRepository) {
    ...
    public void run() {
        ...
        if (machine.isPresent())
        {
            ...
            if(machine.get().getStatus().equals(Status.STOPPED)){
                ErrorMsg errorMsg = new ErrorMsg(this.machineId, "Machine is already stopped","Start",new Date(),this.userId);
                errorMsgRepository.save(errorMsg);
                return;
            }
